...
function doTracking(handlerUrl, siteId, pageId, pageTitle) {
  var url =
    ...
    pageId +
    "&pageTitle=" +
    _uriEncode(pageTitle) +
    "&url=" +
    _uriEncode(document.location) +
    "&referrer=" +
    _uriEncode(document.referrer) +
    "&track=" +
    new Date().getTime();
...
function doClick(
  ...
  var url =
    ...
    _uriEncode(pageTitle) +
    "&clickTitle=" +
    _uriEncode(clickTitle) +
    "&clickText=" +
    _uriEncode(clickText) +
    "&clickUrl=" +
    _uriEncode(clickUrl) +
    "&track=" +
    new Date().getTime();
  ...
  if (!isChrome && isSafari && clickTarget.toLowerCase() != "_blank") {
  ...
  try {
    var the_height =
      document.getElementById(iframeId).contentWindow.document.body
        .scrollHeight;
    var the_height2 =
      document.getElementById(iframeId).contentWindow.document.body.parentNode
        .clientHeight;
...
function _bindKioskMode(pThis) {
  ...
  var vd = m_virtualDirectory;
  if (typeof vd == "undefined" || vd == "") vd = "/";
  if (vd.length > 0 && vd.substring(vd.length - 1, 1) != "/") vd += "/";
  var page = vd + "kioskmode.aspx";
  ...
  window.open(
    page + "?p=" + URL,
    "",
    fullScreen + "toolbar=no,resizable=no" + size,
    false
  );
  return false;
}
function _isSizeClass(cssClass) {
  if (cssClass.indexOf("size-") > -1) return true;
  return false;
}
...
function ajaxHelperCall(method, parameters, callBackFunction, errorCallback) {
  var result = null;
  if (parameters == null || parameters == "") parameters = "";
  else parameters = JSON.stringify(parameters);
  var async = false;
  if (callBackFunction != null) async = true;
  $.ajax({
    async: async,
    type: "POST",
    data: parameters,
    ...
    dataType: "json",
    url: m_virtualDirectory + method,
    success: function (data) {
      result = data.d;
      if (callBackFunction != null) callBackFunction(result);
    },
    error: function (data) {
...
async function asyncAjaxHelperCall(endpoint, data) {
  const response = await fetch(m_virtualDirectory + endpoint, {
    method: "POST",
...
var WTP_Popup = function () {
  var _id = 0;
  var _popupID = "wtp-popup";
  var _iframeID = _popupID + "-iframe";
  var _loadingID = _popupID + "-load";
  var _jqpopupID = "#" + _popupID;
  var _jqiframeID = "#" + _iframeID;
  var _jqloadingID = "#" + _loadingID;
  var _height = null;
  var _heightIFrame = null;
  var _heightMax = null;
  var _width = null;
  var _widthIFrame = null;
  var _widthMax = null;
  var _dialogResized = false;
  var _settings = null;
  var _onLoad;
  var _popupStack = [];
  function onOpen() {
    ...
    _height = popup.prop("offsetHeight");
    _heightIFrame = iframe.height();
    _heightMax = max(_height, _heightIFrame);
    _width = popup.prop("offsetWidth");
    _widthIFrame = iframe.width();
    _widthMax = max(_width, _widthIFrame);
    if (isInPopup()) {
      window.parent.wtpPopup.resizeDialog(_heightMax, _widthMax);
    }
  }
  function onBeforeClose() {
    if (isInPopup()) {
      window.parent.wtpPopup.restoreSizeDialog();
    }
    _popupStack.pop();
    this.isOpened = _popupStack.length > 0;
  }
  function isInPopup() {
  ...
  function centerPopup() {
    $(_jqpopupID).dialog("option", "position", "center");
  }
  function showIframe() {
    $(_jqloadingID).hide();
    $(_jqiframeID).show();
  }
  this.pathImageWaiting = "";
  this.isOpened = false;
  ...
  this.resizeDialog = function (height, width) {
    var doResize = false;
    var pp = $(_jqpopupID).parent(".ui-dialog");
    var p = $(_jqpopupID);
    var dh = Math.round(pp.height() - p.height());
    var dw = Math.round(pp.width() - p.width());
    var h = _height + dh;
    var w = _width + dw;
    if (_height <= height + dh) {
      _dialogResized = true;
      doResize = true;
      h = height + 40;
    }
    ...
    $(_jqpopupID).css({ height: h, width: w });
    $(_jqpopupID)
      .parent(".ui-dialog")
      .css({ height: h + dh, width: w + dw });
    $(_jqiframeID).css({ height: h - dh, width: w - dw });
    centerPopup();
    if (doResize) {
      if (isInPopup()) {
        window.parent.wtpPopup.resizeDialog(height, width);
      }
    } else if (_dialogResized) {
      this.restoreSizeDialog();
    }
  };
  this.restoreSizeDialog = function () {
    if (_dialogResized) {
      _dialogResized = false;
      $(_jqpopupID).css({ height: _height, width: _width });
      $(_jqiframeID).css({ height: _heightIFrame, width: _widthIFrame });
      $(_jqpopupID)
        .parent(".ui-dialog")
  ...
  this.init = function () {
    if (_onLoad) {
      var args = {};
      args.iframeID = _iframeID;
      _onLoad(args);
    }
    showIframe();
    if (
      typeof frames[_iframeID] != "undefined" &&
      typeof frames[_iframeID].loading != "undefined"
    )
      frames[_iframeID].loading = false;
  };
  ...
  this.center = function () {
    var w = parent.wtpPopup.width() - _width;
    var h = parent.wtpPopup.height() - _height;
    if (w > 0) {
      w = w / 2;
      $(_jqpopupID).parent().css({ left: w });
    }
    if (h > 0) {
      h = h / 2;
      if (_settings.title != "") {
        h = h - $(".ui-dialog").find(".ui-dialog-titlebar").height();
        if (h < 0) h = 0;
      }
      $(_jqpopupID).parent().css({ top: h });
    }
  };
  this.open = function (url, options) {
    ...
    _settings = settings;
    var inPopup = isInPopup();
    if (settings.width == "auto" && inPopup) {
      settings.width = parent.wtpPopup.options().width;
    }
    var strW = String(settings.width);
    if (strW.indexOf("%") != -1) {
      var w = strW.substring(0, strW.indexOf("%"));
      w = parseFloat(w) / 100;
      var ww = parseFloat($(window).width());
      w = Math.round(ww * w);
      settings.width = w + "px";
    }
    var strH = String(settings.height);
    if (strH.indexOf("%") != -1) {
      var h = strH.substring(0, strH.indexOf("%"));
      h = parseFloat(h) / 100;
      var wh = parseFloat($(window).height());
      h = Math.round(wh * h);
      settings.height = h + "px";
    }
    if (settings.onLoad) _onLoad = settings.onLoad;
    else _onLoad = null;
    var h = settings.height;
    var w = settings.width;
    _id = _id + 1;
    _popupID = "wtp-popup-" + _id;
    _iframeID = _popupID + "-iframe";
    _loadingID = _popupID + "-load";
    _jqpopupID = "#" + _popupID;
    _jqiframeID = "#" + _iframeID;
    _jqloadingID = "#" + _loadingID;
    var loadingDisplay = "table-cell";
    ...
    $("body").append(
      "<div id='" +
        _popupID +
        "' style='box-sizing;content-box;margin:0px;padding:0px;'><div id='" +
        _loadingID +
        "' style='box-sizing;content-box;display:" +
        loadingDisplay +
        "; vertical-align:middle; text-align:center;'><img src='" +
        tb_pathToImage +
        "' /></div><iframe style='box-sizing;content-box;margin:0px;padding:0px;display:" +
        iframeDisplay +
        "' frameborder='0' hspace='0' id='" +
        _iframeID +
        "' onload='wtpPopup.init()'>pepe</iframe></div>"
    );
    div = $(_jqpopupID);
    iframe = div.find(_jqiframeID);
    iframe.css({ height: settings.height, width: settings.width });
    $(_jqloadingID).css({ height: settings.height, width: settings.width });
    ...
    if (settings.title == "" && !settings.closeButton)
      ...
      dlg
        ...
        .remove();
    _popupStack.push({ popupID: _jqpopupID, iframeID: _jqiframeID });
    return _iframeID;
  };
  this.close = function () {
    var currentPopup = _popupStack[_popupStack.length - 1];
    var div = $(currentPopup.popupID);
    var iframe = div.find(currentPopup.iframeID);
    if (div.length > 0) {
      iframe.attr("src", "");
      div.dialog("close");
    }
...
var wtpPopup = new WTP_Popup();
function wtp_popup(url, options) {
  wtpPopup.open(url, options);
}
function wtp_popupCenter(url, options) {
  var returnValue = wtpPopup.open(url, options);
  wtpPopup.center();
  return returnValue;
}
function wtp_closePopup() {
  if (wtpPopup.isOpened) {
    wtpPopup.close();
  } else {
    if (typeof tb_remove == "function") tb_remove();
  }
  wtpPopup.removeOnLoad();
}
function wtpMessage(text, title, options) {
  $.noty.closeAll();
  $.noty.clearQueue();
  if (title == undefined) title = "";
  if (title != "") text = "<strong>" + title + "</strong><br/>" + text;
  var defaultOptions = {
    text: text,
    layout: "topRight",
    type: "success",
  ...
  options = $.extend({ title: "" }, defaultOptions, options);
  var n = noty(options);
  if (options.timeout)
    setTimeout(function () {
      n.close();
...
function wtpMessageModal(text, title, options) {
  var defaultOptions = {
    type: "warning",
    modal: true,
    timeout: false,
  ...
  return wtpMessage(text, title, options);
}
function wtpMessageWarning(text, title) {
  return wtpMessage(text, title, { type: "warning" });
}
function wtpMessageSuccess(text, title) {
  return wtpMessage(text, title, { type: "success" });
}
function wtpMessageError(text, title) {
  return wtpMessage(text, title, { type: "error" });
}
function wtpMessageAlert(text, title) {
  wtpMessageModal(text, title, {
    buttons: [
      {
  ...
  $.noty.layouts.centerLeft = {
    ...
    container: {
      ...
      style: function () {
        ...
        dupe.remove();
        if ($(this).hasClass("i-am-new")) {
          $(this).css({ top: ($(window).height() - actual_height) / 2 + "px" });
        } else {
          $(this).animate(
            { top: ($(window).height() - actual_height) / 2 + "px" },
            500
...
var ajaxLoginMessagesUserVisibilityHelpers = new (function () {
  var baseUrl = "/LoginMessages/AjaxHelperLoginMessageUserVisibility.asmx/";
  this.save = function (
    loginMessageUserVisibility,
    callBackFunction,
    errorCallback
  ) {
    return ajaxCall(
      "Save",
      { loginMessageUserVisibility: loginMessageUserVisibility },
      callBackFunction,
      errorCallback
    );
  };
  function ajaxCall(method, parameters, callBackFunction, errorCallback) {
    return ajaxHelperCall(
      baseUrl + method,
      parameters,
      callBackFunction,
      errorCallback
...
function saveLoginMessagesUserVisibility(loginMessageId, login) {
  var loginMessageUserVisibility = {
    LoginMessageId: loginMessageId,
    Login: login,
    Visible: false,
  };
  ajaxLoginMessagesUserVisibilityHelpers.save(
    loginMessageUserVisibility,
    this.onSaveMessageSuccessfully,
    this.onSaveMessageError
  );
}
function showLoginMessages(messages) {
  this.loginMessages = messages;
  this.currentPosition = 0;
  this.currentMessage = this.loginMessages[this.currentPosition];
  showMessage(this.currentMessage);
}
function showMessage(message) {
  if (
    typeof message.PageUrl !== "undefined" &&
    message.PageUrl !== null &&
    message.PageUrl.length > 0
  ) {
    $("#ifrPage").prop("src", message.PageUrl);
    this.showRow("page");
  } else {
    var decodedMessageContent = getDecodedMessaggeContent(message.Message);
    if (message.IsHtml) {
      $("#spnMessage").html(decodedMessageContent);
      $("#spnMessage").show();
      $("#txtMessage").hide();
    } else {
      $("#txtMessage").val(decodedMessageContent);
      $("#txtMessage").show();
      $("#spnMessage").hide();
    }
    this.showRow("message");
  }
  $("#modalTitle").text(message.Title);
  a = $("#loginMessageModal").modal();
}
function getDecodedMessaggeContent(content) {
  var decodedMessage = content;
  try {
    decodedMessage = decodeURIComponent(content);
  } catch (e) {
    decodedMessage = unescape(content);
  }
...
function showRow(rowName) {
  if (rowName === "page") {
    ...
    $("#pageRow").css("display", "block");
  } else {
    $("#pageRow").css("display", "none");
    $("#messageRow").css("display", "block");
  }
}
function onClosePopup(disableMessage) {
  this.clearFields();
  if (disableMessage) {
    var message = this.loginMessages[this.currentPosition];
    this.saveLoginMessagesUserVisibility(message.Id, message.Login);
  }
  var nextPosition = this.currentPosition + 1;
  if (nextPosition < this.loginMessages.length) {
...
function onSaveMessageSuccessfully(result) {
  $.unblockUI();
  if (!result.Success)
    bootbox.alert(
      "The following errors were found:\n" +
        result.ErrorMessages.map(function (x) {
          return "- " + x.Message;
        }).join("\n")
    );
}
function onSaveMessageError(error) {
  $.unblockUI();
  bootbox.alert("There was an error saving the login messages user visibility");
}
(function ($) {
